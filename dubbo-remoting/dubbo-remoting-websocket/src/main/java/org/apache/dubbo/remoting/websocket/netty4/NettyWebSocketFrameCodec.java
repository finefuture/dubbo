...
public class NettyWebSocketFrameCodec extends ChannelDuplexHandler {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof BinaryWebSocketFrame || msg instanceof TextWebSocketFrame) {
            ...
            onCloseFrame(ctx, (CloseWebSocketFrame) msg);
        } else {
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
        if (msg instanceof Http2OutputMessage) {
            WebSocketFrame webSocketFrame = encodeWebSocketFrame((Http2OutputMessage) msg);
    ...
    private void onCloseFrame(ChannelHandlerContext ctx, CloseWebSocketFrame closeWebSocketFrame) throws Exception {
        if (closeWebSocketFrame.statusCode() != WebSocketCloseStatus.NORMAL_CLOSURE.code()) {
            super.userEventTriggered(ctx, new DefaultHttp2ResetFrame(closeWebSocketFrame.statusCode()));
            return;
        }
        super.channelRead(ctx, new Http2InputMessageFrame(StreamUtils.EMPTY, true));
    }
